...
#define GLFW_EXPOS_NATIVE_WIN32
...
const std::string MODEL_PATH = std::string(RESOURCES_PATH) + "models/viking_room.obj";
const std::string TEXTURE_PATH = std::string(RESOURCES_PATH) + "textures/viking_room.png";
...
// required device extensions lists
const std::vector<const char*> deviceExtensions = {
	VK_KHR_SWAPCHAIN_EXTENSION_NAME
};

// check whether to enable/disable validation layers
#ifdef NDEBUG
...
private:
	...
	void mainLoop() {
		...
		// wait for logical device to finish operations before
		// exiting mainLoop
	...
	void cleanup() {
		...
		// destroy surface (do this before Vulkan instance)
	...
	void createGraphicsPipeline() {
		auto vertShaderCode = readFile(std::string(RESOURCES_PATH) + "/shaders/vert.spv");
		auto fragShaderCode = readFile(std::string(RESOURCES_PATH) + "/shaders/frag.spv");
	...
	void createFramebuffers() {
		...
		// iterate through all image views (create framebuffers for each)
		for (size_t i = 0; i < swapChainImageViews.size(); ++i) {
